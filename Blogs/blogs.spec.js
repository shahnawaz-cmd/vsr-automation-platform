...
class Blogs_Page {
  ...
  async toggleMenu() {
    ...
    const isMenuOpen = async () => await this.page.locator('nav.mobile-menu:not(.hidden), .mobile-menu').first().isVisible();
...
const blogPageUrls = [
    'https://vsr.accessautohistory.com/blog'
];
...
for (const targetUrl of blogPageUrls) {
    test.describe(`Blogs Module - ${targetUrl}`, () => {
        for (const viewport of viewports) {
            test.describe(`Viewport: ${viewport.width}x${viewport.height}`, () => {
                ...
                test.beforeEach(async ({ page }) => {
                    ...
                    targetPage = new Blogs_Page(page, targetUrl);
        ...
        test('One-Time: Meta, Heading, Link Integrity, and CTA Report', async ({ page }, testInfo) => {
            test.setTimeout(90000);
            const targetPage = new Blogs_Page(page, targetUrl);
            await targetPage.goto();
            ...
            // Update Pagination Report with more inclusive selector
            const pagination = await page.locator('nav[aria-label="Pagination"] a, .pagination a, .page-link, [aria-label*="Page"]').all();
            const paginationCount = pagination.length;
            await testInfo.attach('Pagination Report', { body: JSON.stringify({ totalPaginationLinks: paginationCount }, null, 2), contentType: 'application/json' });
            });
            });}
