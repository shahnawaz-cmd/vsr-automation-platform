...
const stickerPageUrl = 'https://vsr.accessautohistory.com/window-stickers';
const viewports = [
    ...
    for (const viewport of viewports) {
        test.describe(`Viewport: ${viewport.width}x${viewport.height}`, () => {
            ...
            test('Batch: All Responsive Cases', async () => {
                ...
                // Allow tables to be non-scrollable if they fit the screen on larger viewports
                const tableScrollable = await targetPage.isTableScrollable();
                if (viewport.width < 768) {
                    expect(tableScrollable, 'C01 Failed').toBe(true);
                }
