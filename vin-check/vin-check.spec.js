...
const vinCheckUrls = [
    'https://vsr.accessautohistory.com/vin-check'
];
...
for (const targetUrl of vinCheckUrls) {
    test.describe(`VIN Check Module - ${targetUrl}`, () => {
        for (const viewport of viewports) {
            test.describe(`Viewport: ${viewport.width}x${viewport.height}`, () => {
                ...
                test.beforeEach(async ({ page }) => {
                    ...
                    targetPage = new VIN_Check(page, targetUrl);
        ...
        test('One-Time: Meta, Heading, Link Integrity, and CTA Report', async ({ page }, testInfo) => {
            test.setTimeout(90000);
            const targetPage = new VIN_Check(page, targetUrl);
            await targetPage.goto();
